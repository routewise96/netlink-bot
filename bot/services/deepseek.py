...
1. Установка и настройка приложений:
   - Happ (iPhone, Android) — ссылка копируется автоматически, в Happ нажать "+" → "Из буфера обмена"
...
- "Яндекс не работает" → Отключите "Блокировать рекламу" в настройках Hiddify (Windows). На iPhone/Android через Happ Яндекс должен работать без проблем.
