...
def _get_device(sub_id: str) -> dict | None:
    """Look up by sub_id. Filters out expired temp links (race protection
    against cleanup_temp.py running once a minute)."""
    conn = sqlite3.connect(f"file:{NETLINK_DB_PATH}?mode=ro", uri=True)
    try:
        row = conn.execute(
            "SELECT uuid, status, is_temp, expires_at FROM user_devices "
            "WHERE sub_id = ?",
            (sub_id,),
        ).fetchone()
    finally:
    ...
    if not row:
        return None
    uuid_, status, is_temp, expires_at = row
    if is_temp and expires_at:
        try:
            if datetime.fromisoformat(expires_at) < datetime.now():
                return None
        except ValueError:
            return None
    return {"uuid": uuid_, "status": status}
